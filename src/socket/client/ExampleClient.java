...
public class ExampleClient extends BasicClient
{
	...
	public boolean sendHighScoreFrame( String username, int score ) 
	{
		HighScoreFrame highScoreFrame = new HighScoreFrame();
		
		highScoreFrame.setUsername( username );
		highScoreFrame.setScore( score );
		
		try {
			this.outputStream.writeObject( highScoreFrame );
		} catch (IOException e) {
			System.out.println("blad1");
			e.printStackTrace();
			System.out.println( "Klient: Nie uda³o zapisaæ sie wyniku na serwerze." );
			return false;
		}
		return true;
		// Utils.sleep( 1000 );
		...
		/*{
			...
			try {
				resultFrame = (ResultFrame) this.inputStream.readObject();
			...
			if ( resultFrame!=null && resultFrame.isCorrect() )
			{
				System.out.println( "Klient: Wynik zapisany na serwerze!" );
				return true;
			}
		}*/
	}
	...
	public ArrayList<HighScoreFrame> getListOfHighscores() {
